...
class AsyncModalOperatorMixin:
    ...
    def modal(self, context, _):
        ...
        if task and (task.done() or task.cancelled()):
            self.log.info(
                'Task was cancelled/done {}/{}'.format(task.cancelled(), task.done()))
            context.window_manager.event_timer_remove(self.timer)
            # noinspection PyBroadException
            try:
    ...
    def _new_async_task(self, async_task: typing.Coroutine):
        ...
        if self.single_task:
            async_task_map[self.task_id] = self.async_task

        ensure_async_loop()

    def _stop_prev_async_task(self):
        async_task = async_task_map.get(self.task_id)
        if async_task is None:
            return
        cancelled = async_task.cancel()
        if not cancelled:
            self.log.info(
                "Previous Task had Completed with id {}".format(self.task_id))
        else:
        ...
        if not async_task.done():
            ...
            try:
                self.log.info(
                    "Wait task to complete, id {}".format(self.task_id))
